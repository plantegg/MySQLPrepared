...
public class PreparedStatementPerformanceTest {
    ...
    private static final int TEST_ROUNDS = Integer.parseInt(
            System.getProperty("test.rounds", System.getenv().getOrDefault("TEST_ROUNDS", "2"))); // 执行次数（默认2次）
    private static final int PARAM_SPACES = Integer.parseInt(
            System.getProperty("param.spaces", System.getenv().getOrDefault("PARAM_SPACES", "128"))); // 参数之间的空格数量（默认128）
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (DB_HOST == null || DB_PORT == null || DB_NAME == null || DB_USER == null || DB_PASSWORD == null) {
            ...
            System.err.println(
                    "java -Ddb.host=localhost -Ddb.port=3306 -Ddb.name=test -Ddb.user=root -Ddb.password=password PreparedStatementPerformanceTest");
            System.exit(1);
        }
    ...
    static void testPreparedStatement(boolean useServerPrepStmts) throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(url, DB_USER, DB_PASSWORD)) {
            ...
            // 构建IN查询SQL（参数之间的空格数量完全由--spaces参数控制）
            StringBuilder inClause = new StringBuilder();
            String spaces = PARAM_SPACES > 0 ? " ".repeat(PARAM_SPACES) : ""; // 可配置的空格
            for (int i = 0; i < ids.size(); i++) {
                if (i == 0) {
                    inClause.append("?");
                } else {
                    inClause.append("," + spaces + "?");
                }
            }
            ...
            System.out.println("参数空格数: " + PARAM_SPACES);
